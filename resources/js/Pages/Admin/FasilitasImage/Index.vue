<template>
  <AdminLayout :title="`Galeri Foto - ${fasilitas.nama}`">
    <div class="space-y-6">
      <!-- Header -->
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-xl font-bold">Fasilitas: {{ fasilitas.nama }}</h2>
          <p>ID: {{ fasilitas.id }}</p>
        </div>
        <Link :href="route('fasilitas.index')" class="btn btn-primary">Back</Link>
      </div>
        ...
        <SwiperSlide v-for="img in images" :key="img.id">
          <div class="relative">
            <a :href="'/' + img.foto" target="_blank">
              <img
                :src="'/' + img.foto"
                class="w-full max-h-[500px] object-contain rounded"
              />
            </a>
            <button
              @click="hapus(img.id)"
...
function submit() {
  ...
  router.post(`/admin/fasilitas/${props.fasilitas.id}/images`, formData, {
    forceFormData: true,
    preserveScroll: true,
    onFinish: () => {
...
function hapus(id) {
  ...
  router.delete(route('fasilitasimage.destroy', id), {
    preserveScroll: true,
  })
}
</script>
