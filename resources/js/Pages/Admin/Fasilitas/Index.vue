<template>
  <AdminLayout title="Data Fasilitas">
    ...
    <div class="bg-white p-6 rounded shadow">
      ...
      <!-- DataTables target -->
      <table ref="dtRef" id="fasilitasTable" class="min-w-full table-auto border text-sm">
        <thead>
          ...
          <tr v-for="item in rows" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-2 border">
              <Link
                as="button"
                type="button"
                :href="route('fasilitasimage.index', { fasilitasId: item.id })"
            ...
            <td class="px-4 py-2 border">
              <div class="flex gap-2">
                <Link :href="route('fasilitas.edit', item.id)" class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded btn-xs">
...
const props = defineProps({ fasilitas: { type: Object, default: () => ({ data: [] }) } })
/* dukung array langsung atau paginator Laravel */
const rows = computed(() => Array.isArray(props.fasilitas?.data) ? props.fasilitas.data : (props.fasilitas || []))
