      ...
      <form @submit.prevent="submit" enctype="multipart/form-data">
        ...
        <div class="mb-4 flex flex-col md:flex-row md:items-start gap-4">
          ...
          <div class="w-full md:w-1/3">
            ...
            <div v-else-if="typeof form.foto === 'string' && form.foto !== ''" class="mt-2">
              <img :src="`/storage/${form.foto}`" alt="Foto Lama" class="w-full max-w-[120px] rounded border shadow" />
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <div v-if="form.errors.tinggal_dengan" class="text-red-500 text-sm mt-1">{{ form.errors.tinggal_dengan }}
            </div>
          </div>
          ...
          <div>
            ...
            <div v-if="form.errors.jarak_ke_madrasah" class="text-red-500 text-sm mt-1">{{ form.errors.jarak_ke_madrasah
              }}</div>
          </div>
        </div>
        ...
        <div class="mt-8">
          ...
          <div v-for="(ortu, index) in form.orang_tuas" :key="index" class="mb-6 p-4 border rounded bg-gray-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label class="block">Nama</label><input v-model="ortu.nama" type="text" class="form-input w-full" />
              </div>
              ...
              <div><label class="block">NIK</label><input v-model="ortu.nik" type="text" class="form-input w-full" />
              </div>
              <div><label class="block">Tempat Lahir</label><input v-model="ortu.tempat_lahir" type="text"
                  class="form-input w-full" /></div>
              <div><label class="block">Tanggal Lahir</label><input v-model="ortu.tanggal_lahir" type="date"
                  class="form-input w-full" /></div>
              <div><label class="block">Pendidikan</label><input v-model="ortu.pendidikan" type="text"
                  class="form-input w-full" /></div>
              <div><label class="block">Pekerjaan</label><input v-model="ortu.pekerjaan" type="text"
                  class="form-input w-full" /></div>
              <div><label class="block">Penghasilan</label><input v-model="ortu.penghasilan" type="text"
                  class="form-input w-full" /></div>
              <div><label class="block">No HP</label><input v-model="ortu.no_hp" type="text"
                  class="form-input w-full" /></div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="block">Nama Wali</label><input v-model="form.wali.nama" type="text"
                class="form-input w-full" /></div>
            <div><label class="block">Hubungan Keluarga</label><input v-model="form.wali.hubungan_keluarga" type="text"
                class="form-input w-full" /></div>
          </div>
        </div>
...
const form = useForm({
  ...
  imunisasi: props.pendaftar.imunisasi || [],
