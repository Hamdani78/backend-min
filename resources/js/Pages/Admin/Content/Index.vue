<script setup>
import AdminLayout from '@/Layouts/AdminLayout.vue'
import { Link, router } from '@inertiajs/vue3'
import { computed, onMounted, onBeforeUnmount, nextTick, ref, watch } from 'vue'

const props = defineProps({
  // Bisa array biasa atau paginator Laravel
  contents: { type: [Array, Object], default: () => ([]) }
})

const rows = computed(() => {
  if (Array.isArray(props.contents)) return props.contents
  if (props.contents?.data && Array.isArray(props.contents.data)) return props.contents.data
  return []
})

/* ===== DataTables ===== */
import DataTable from 'datatables.net-dt'
import 'datatables.net-dt/css/dataTables.dataTables.css'
import 'datatables.net-responsive-dt'
import 'datatables.net-responsive-dt/css/responsive.dataTables.css'
import 'datatables.net-buttons-dt'
import 'datatables.net-buttons-dt/css/buttons.dataTables.css'
import 'datatables.net-buttons/js/buttons.colVis.mjs'
import 'datatables.net-buttons/js/buttons.html5.mjs'
import 'datatables.net-buttons/js/buttons.print.mjs'

import JSZip from 'jszip'
window.JSZip = JSZip
import pdfMake from 'pdfmake/build/pdfmake'
import { vfs as pdfVfs } from 'pdfmake/build/vfs_fonts'
pdfMake.vfs = pdfVfs
window.pdfMake = pdfMake

const dtRef = ref(null)
let dt

function initDT () {
  if (!dtRef.value) return
  if (dt) dt.destroy()
  dt = new DataTable(dtRef.value, {
    pageLength: 10,
    lengthChange: false,
    responsive: true,
    autoWidth: false,
    layout: { topStart: { buttons: ['copy','csv','excel','pdf','print','colvis'] } },
    columnDefs: [
      { targets: [3], orderable: false },
      { targets: [3], searchable: false }
    ]
  })
}

onMounted(async () => { await nextTick(); initDT() })
onBeforeUnmount(() => { if (dt) dt.destroy() })
watch(rows, async () => { await nextTick(); initDT() })

function hapus(id) {
  if (confirm('Hapus konten ini?')) {
    router.delete(route('content.destroy', id))
  }
}
</script>

<template>
  <AdminLayout title="Kelola Konten">
    <div v-if="$page.props.flash?.success" class="bg-blue-100 text-blue-800 p-4 rounded mb-4">
      {{ $page.props.flash.success }}
    </div>

    <div class="bg-white p-6 rounded shadow">
      <!-- HEADER: satu saja -->
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-lg font-semibold">Data Table Konten</h2>

        <!-- Tombol Tambah menuju content.create -->
        <Link
          as="button"
          type="button"
          :href="route('content.create')"
          class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded inline-flex items-center"
        >
          <i class="fa fa-plus mr-2"></i> Tambah
        </Link>
      </div>

      <table ref="dtRef" id="contentTable" class="min-w-full table-auto border text-sm">
        <thead>
          <tr class="bg-gray-100 text-left">
            <th class="px-4 py-2 border">Judul</th>
            <th class="px-4 py-2 border">Slug</th>
            <th class="px-4 py-2 border">Diubah</th>
            <th class="px-4 py-2 border w-32">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in rows" :key="item.id" class="hover:bg-gray-50">
            <td class="px-4 py-2 border">
              <span class="truncate-1">{{ item.title }}</span>
            </td>
            <td class="px-4 py-2 border font-mono text-xs">
              <span class="truncate-1">{{ item.slug }}</span>
            </td>
            <td class="px-4 py-2 border">
              {{ new Date(item.updated_at).toLocaleString() }}
            </td>
            <td class="px-4 py-2 border">
              <div class="flex gap-2">
                <!-- GUNAKAN content.edit, bukan admin.content.edit -->
                <Link
                  :href="route('content.edit', item.id)"
                  class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded btn-xs"
                >
                  <i class="fa fa-edit"></i>
                </Link>
                <button
                  @click="hapus(item.id)"
                  class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded btn-xs"
                >
                  <i class="fa fa-trash"></i>
                </button>
              </div>
            </td>
          </tr>

          <tr v-if="!rows.length">
            <td colspan="4" class="px-4 py-6 text-center text-gray-500">Belum ada data konten.</td>
          </tr>
        </tbody>
      </table>
    </div>
  </AdminLayout>
</template>

<style scoped>
:deep(table.dataTable){
  border-collapse: collapse !important;
  border-spacing: 0 !important;
  table-layout: fixed;
}
:deep(table.dataTable thead th),
:deep(table.dataTable tbody td){
  padding: 6px 10px;
  vertical-align: middle;
}
/* lebar kolom */
:deep(table.dataTable th:nth-child(1)) { width: 260px; }
:deep(table.dataTable th:nth-child(2)) { width: 160px; }
:deep(table.dataTable th:nth-child(3)) { width: 200px; }
:deep(table.dataTable th:nth-child(4)) { width: 120px; }

.truncate-1{ display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.btn-xs{ padding:4px 8px; line-height:1; }
</style>
